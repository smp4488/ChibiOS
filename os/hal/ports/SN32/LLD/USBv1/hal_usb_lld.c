...
#if SN32_USB_USE_ISOCHRONOUS
  uint32_t epr = SN32_USB->EPR[ep];
...
#if SN32_USB_USE_USB1 || defined(__DOXYGEN__)
#if SN32_USB1_HP_NUMBER != SN32_USB1_LP_NUMBER
#if SN32_USB_USE_ISOCHRONOUS
/**
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(SN32_USB1_HP_HANDLER) {
  uint32_t istr;
...
#endif /* SN32_USB_USE_ISOCHRONOUS */
#endif /* SN32_USB1_LP_NUMBER != SN32_USB1_HP_NUMBER */
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(SN32_USB1_LP_HANDLER) {
  uint32_t istr;
  USBDriver *usbp = &USBD1;
  ...
  if (istr & ISTR_SUSP) {
    SN32_USB->CNTR |= CNTR_FSUSP;
#if SN32_USB_LOW_POWER_ON_SUSPEND
    SN32_USB->CNTR |= CNTR_LP_MODE;
#endif
    SN32_USB->ISTR = ~ISTR_SUSP;
...
#endif
    SN32_USB->ISTR = ~ISTR_WKUP;
  }

  /* SOF handling.*/
  if (istr & ISTR_SOF) {
    _usb_isr_invoke_sof_cb(usbp);
    SN32_USB->ISTR = ~ISTR_SOF;
  }
...
#if SN32_USB1_HP_NUMBER != SN32_USB1_LP_NUMBER
      nvicEnableVector(SN32_USB1_HP_NUMBER, SN32_USB_USB1_HP_IRQ_PRIORITY);
#endif
      nvicEnableVector(SN32_USB1_LP_NUMBER, SN32_USB_USB1_LP_IRQ_PRIORITY);
...
void usb_lld_stop(USBDriver *usbp) {
  ...
  if (usbp->state == USB_STOP) {
    ...
    if (&USBD1 == usbp) {
#if SN32_USB1_HP_NUMBER != SN32_USB1_LP_NUMBER
      nvicDisableVector(SN32_USB1_HP_NUMBER);
#endif
...
void usb_lld_reset(USBDriver *usbp) {
  ...
  SN32_USB->BTABLE = BTABLE_ADDR;
  SN32_USB->ISTR   = 0;
  SN32_USB->DADDR  = DADDR_EF;
...
void usb_lld_set_address(USBDriver *usbp) {
  ...
  SN32_USB->DADDR = (uint32_t)(usbp->address) | DADDR_EF;
}
...
usbepstatus_t usb_lld_get_status_out(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (SN32_USB->EPR[ep] & EPR_STAT_RX_MASK) {
...
usbepstatus_t usb_lld_get_status_in(USBDriver *usbp, usbep_t ep) {
  ...
  (void)usbp;
  switch (SN32_USB->EPR[ep] & EPR_STAT_TX_MASK) {
...
void usb_lld_clear_out(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((SN32_USB->EPR[ep] & EPR_STAT_RX_MASK) != EPR_STAT_RX_VALID)
...
void usb_lld_clear_in(USBDriver *usbp, usbep_t ep) {
  ...
  /* Makes sure to not put to NAK an endpoint that is already
     transferring.*/
  if ((SN32_USB->EPR[ep] & EPR_STAT_TX_MASK) != EPR_STAT_TX_VALID)
