...
// {
// 		LDR			r0,=0x40060024 //”IVTM address”
//     STR			r1,[r0]
// 		//Pop{r0-r6, PC}
...
void USB_EP0SetupEvent (void)
{
	...
	SUSB_EUME_DATA 	*psDevice =&sUSB_EumeData;
	uint32_t	*pRAM = &USB_SRAM_EP0_W0;
	uint32_t	wCmd;

	__USB_CLRINSTS((mskEP0_SETUP|mskEP0_PRESETUP|mskEP0_OUT_STALL|mskEP0_IN_STALL));		// Clear ENDP0_SETUP & ENDP0_PRESETUP = 0
	USB_EPnNak(USB_EP0);														// keep EP0	NAK
	...
	if (!(SN_USB->INSTS & mskERR_SETUP))
	{
		...
		// destination buffer   // source buffer  // bytes to copy */

		memcpy (psDevice, pRAM, 8);								//save EP0 SETUP DATA to sUSB_EumeData
		/*The memcpy function copies len bytes from src to dest.
		 If these memory buffers overlap, the memcpy function cannot guarantee that bytes in src are copied
			to dest before being overwritten. If these buffers do overlap, use the memmove function.*/

		wCmd = (psDevice->bUSB_bmRequestType) & REQUEST_TYPE_MASK;
...
{
	uint32_t* locptr = (uint32_t*)0x20000000 ; 	//Never remove!!!!
	uint32_t	*pUsbSram = &USB_SRAM_EP0_W0;			//Never remove!!!!
	...
	{	//The IN token of set report feature command END.
		...
		{		//check ISP password
				for(*pUsbSram=0; *pUsbSram <256 ;(*pUsbSram)++) //Never remove!!!!
				{
					*(locptr+ (*pUsbSram))=0;	//Never remove!!!!
				}
...
void USB_GetStatusEvent(void)
{
	...
	else if (wCmdtype == REQUEST_TO_INTERFACE)	// Interface
	{
		if ((sUSB_EumeData.wUSB_SetConfiguration == USB_CONFIG_VALUE) && (sUSB_EumeData.bUSB_wIndexH == 0))
		{
				...
				if (sUSB_EumeData.bUSB_wIndexL == USB_INTERFACE_0)
...
void	USB_ClearFeatureEvent(void)
{
	...
	else if (wCmdtype == REQUEST_TO_ENDPOINT)	// Endpoint
	{
		if (sUSB_EumeData.bUSB_wValueL == USB_FEATURE_ENDPOINT_STALL)
		{
			...
			#if (USB_LIBRARY_TYPE == USB_KB_MOUSE_TYPE1)
				else if (wEPNum <= USB_EP3)
			#elif (USB_LIBRARY_TYPE == USB_MOUSE_TYPE)
				else if (wEPNum == USB_EP1)
			#elif (USB_LIBRARY_TYPE == USB_KB_MOUSE_TYPE2)
				else if (wEPNum <= USB_EP2)
			#endif
				{
...
void	USB_SetFeatureEvent(void)
{
	...
	else if (wCmdtype == REQUEST_TO_ENDPOINT)	// Endpoint
	{
		if (sUSB_EumeData.bUSB_wValueL == USB_FEATURE_ENDPOINT_STALL)
		{
			wEPNum = sUSB_EumeData.bUSB_wIndexL & USB_ENDPOINT_NUM;
		#if (USB_LIBRARY_TYPE == USB_KB_MOUSE_TYPE1)
			if ((wEPNum >= USB_EP1) && (wEPNum <= USB_EP3))
		#elif (USB_LIBRARY_TYPE == USB_MOUSE_TYPE)
			if (wEPNum == USB_EP1)
		#elif (USB_LIBRARY_TYPE == USB_KB_MOUSE_TYPE2)
			if ((wEPNum >= USB_EP1) && (wEPNum <= USB_EP2))
		#endif
			{
				if (sUSB_EumeData.wUSB_SetConfiguration == USB_CONFIG_VALUE)
				{
		...
		else if (sUSB_EumeData.bUSB_wValueL == USB_CONFIG_VALUE)
		{
		...
		#endif
			{
				*(pUsbRam+i) = USB_EPn_NON_HALT;		// wUsb_EndpHaltn = 0
				...
				{
					USB_EPnNak(i);					// Enable EP1~EP6
					if ( (SN_USB->CFG>>(i-1)) & mskEP1_DIR) //if EP1~EP6 is OUT direction
						USB_EPnAck(i, 0);			// set EP1~EP6 ACK
				}
			}
			sUSB_EumeData.wUSB_Status |= mskSETUP_IN;			// Set Setup_IN = 1
			USB_EPnAck(USB_EP0,0);					// EP0 ACK 0 byte
	...
	if (sUSB_EumeData.wUSB_SetConfiguration == USB_CONFIG_VALUE)
	{
		if (sUSB_EumeData.bUSB_bmRequestType == (REQUEST_DEVICE_TO_HOST|REQUEST_TO_INTERFACE))
		{
		...
		#endif
			{
				...
				USB_SRAM_EP0_W0 = *(&wUSB_IfAlternateSet[0]+sUSB_EumeData.bUSB_wIndexL);
				sUSB_EumeData.wUSB_Status |= mskSETUP_OUT;	// Set Setup_OUT = 1
				USB_EPnAck(USB_EP0,1);				// EP0 ACK 1 byte
				return;
			}
...
void	USB_SetInterfaceEvent(void)
{
	...
	{
		pUsbRam = &wUSB_EndpHalt[0];
		for (i=USB_EP0; i<=USB_EP6; i++)
		{
			*(pUsbRam+i) = USB_EPn_NON_HALT;	// wUsb_EndpHalt0~wUsb_EndpHalt6 = 0
		}
	#if (USB_LIBRARY_TYPE == USB_KB_MOUSE_TYPE1)
		if (sUSB_EumeData.bUSB_wIndexL <= USB_INTERFACE_2)
	#elif (USB_LIBRARY_TYPE == USB_MOUSE_TYPE)
